...
//import com.example.myapplication.env.Recognition;
import com.example.myapplication.Regconition;
...
public class MultiBoxTracker {
  ...
  private ArrayList <RectF> results = new ArrayList<RectF>();
  ...
  public synchronized void trackResults(
          final ArrayList<Regconition> results, final byte[] frame) {
//    logger.i("Processing %d results from %d", results.size(), timestamp);
    processResults(results, frame);
//    drawBox(canvas);
  }
  public synchronized void drawBox(final Canvas canvas) {
    ...
    for (final RectF box : results) {

//      getFrameToCanvasMatrix().mapRect(trackedPos);
      boxPaint.setColor(Color.RED);

      getFrameToCanvasMatrix().mapRect(box);
      canvas.drawRect(box, boxPaint);
      final String labelString = "Trump 1.00";
      borderedText.drawText(canvas, box.left , box.bottom, labelString);
    }
  }
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if(trackedPos.bottom < frameHeight - 100){
        trackedPos.top += 200;
        trackedPos.bottom += 200;
      }
  ...
  public synchronized void onFrame(
    ...
    objectTracker.nextFrame(frame, null,0, null, true);
  ...
  private void processResults(final ArrayList<Regconition> results, final byte[] originalFrame) {
    final List<Pair<Float,Regconition>> rectsToTrack = new LinkedList<Pair<Float, Regconition>>();


    screenRects.clear();
    final Matrix rgbFrameToScreen = new Matrix(getFrameToCanvasMatrix());

    for (final Regconition result : results) {
      if (result == null) {
        continue;
      }
      final RectF detectionFrameRect = result.box;

      final RectF detectionScreenRect = new RectF();
      rgbFrameToScreen.mapRect(detectionScreenRect, detectionFrameRect);

      screenRects.add(new Pair<Float, RectF>((float) 1.0, detectionScreenRect));
      ...
      rectsToTrack.add(new Pair<Float, Regconition>(result.confident, result));
    }
    ...
    if (objectTracker == null) {
      ...
      for (final Pair<Float, Regconition> potential : rectsToTrack) {
        final TrackedRecognition trackedRecognition = new TrackedRecognition();
        trackedRecognition.detectionConfidence = potential.first;
        trackedRecognition.location = potential.second.box;
        trackedRecognition.trackedObject = null;
        trackedRecognition.title = potential.second.name;
    ...
    for (final Pair<Float, Regconition> potential : rectsToTrack) {
      handleDetection(originalFrame, potential);
    }
  }

  private void handleDetection(
      final byte[] frameCopy,  final Pair<Float, Regconition> potential) {
    final ObjectTracker.TrackedObject potentialObject =

        objectTracker.trackObject(potential.second.box, frameCopy);
